...
void comm(int N_global, double **u, MPI_Comm comm_cart)
{
    ...
    /* Shift right (fill left ghost cells) : Send Nx from left grid to 0
       of the right grid */
    {        
        int disp = 1;
        int dir = 0;
        for(int j = 0; j < Ny+1; j++)
            ghost_buff[0][j] = u[Nx][j];
    ...
    /* Shift up (fill bottom ghost cells) : Send Ny from bottom grid to 0
       of the bottom grid */
    {
        int disp = 1;
        int dir = 1;
        for(int i = 0; i < Nx+1; i++)
            ghost_buff[0][i] = u[i][Ny];
